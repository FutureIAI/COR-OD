...
class DDetect(nn.Module):
    ...
    def forward(self, x, targets_tensor):
        ...
        if self.training:
            ...
            self.anchors, self.strides = (x.transpose(0, 1) for x in make_anchors(x, self.stride, 0.5)) #self.anchors -> (2, 6400)
            self.shape = shape
...
class DDetectx(nn.Module):
    ...
    def __init__(self, nc=80, ch=(), inplace=True):  # detection layer
        ...
        self.cv3 = nn.ModuleList(
            nn.Sequential(Conv(x, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc, 1)) for x in ch)
        self.dfl = DFL(self.reg_max) if self.reg_max > 1 else nn.Identity()
        self.ct_fusion = Ct_fusion(self.nc, self.training, 0.25, self.reg_max)

    def forward(self, x, targets_tensor):
        if targets_tensor == None:
            targets_tensor = torch.zeros(24, 24)
            targets_tensor = targets_tensor.to(device=x[0].device)
        shape = x[0].shape  # BCHW
        for i in range(self.nl):
            x[i] = torch.cat((self.cv2[i](x[i]), self.cv3[i](x[i])), 1)
        # z = torch.cat([xi.view(shape[0], self.no, -1) for xi in x], 2)
        x, feat_tensor = self.ct_fusion(x, targets_tensor)
        if self.training:
            return (x, feat_tensor)
        elif self.dynamic or self.shape != shape:
            self.anchors, self.strides = (x.transpose(0, 1) for x in make_anchors(x, self.stride, 0.5)) #self.anchors -> (2, 8400) self.stride -> [8, 16, 32]
            self.shape = shape

        box, cls = torch.cat([xi.view(shape[0], self.no, -1) for xi in x], 2).split((self.reg_max * 4, self.nc), 1)
        dbox = dist2bbox(self.dfl(box), self.anchors.unsqueeze(0), xywh=True, dim=1) * self.strides
        y = torch.cat((dbox, cls.sigmoid()), 1)
        return (y, feat_tensor) if self.export else (y, x, feat_tensor)
...
class BaseModel(nn.Module):
    ...
    def _forward_once(self, x, targets_tensor=None, profile=False, visualize=False):
        y, dt = [], []  # outputs
        for m in self.model:
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, DualDetect, TripleDetect, DDetect, DDetectx, DualDDetect, TripleDDetect, Segment, DSegment, DualDSegment, Panoptic)):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolo.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect, DDetect, DDetectx, Segment, DSegment, Panoptic)):
            s = 256  # 2x min stride
            m.inplace = self.inplace
            forward = lambda x: self.forward(x)[0] if isinstance(m, (Segment, DSegment, Panoptic)) else self.forward(x)
            if isinstance(m, (Detect, DDetectx, Segment, DSegment, Panoptic)):
                m.stride = torch.tensor([s / x.shape[-2] for x in forward(torch.zeros(1, ch, s, s))[0]])  # forward ch->3 s->256
            else:
    ...
    def forward(self, x, targets_tensor=None, augment=False, profile=False, visualize=False):
        if augment:
            return self._forward_augment(x)  # augmented inference, None
        return self._forward_once(x, targets_tensor, profile, visualize)  # single-scale inference, train
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
            ...
            c2 = ch[f[-1]]
        # TODO: channel, gw, gd
        elif m in {Detect, DualDetect, TripleDetect, DDetect, DDetectx, DualDDetect, TripleDDetect, Segment, DSegment, DualDSegment, Panoptic}:
