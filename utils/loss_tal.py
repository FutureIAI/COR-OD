...
class ComputeLoss:
    ...
    def __call__(self, p, targets, feat_tensor=None, current_targets=None, img=None, epoch=0):
        ...
        target_labels, target_bboxes, target_scores, fg_mask = self.assigner(
            ...
            gt_bboxes,
            mask_gt)
        cls = target_scores.permute(0, 2, 1)
        xc = cls.amax(1) > 0.25
        # max_index = torch.argmax(cls, dim=1, keepdim=True)
        expanded_mask = xc.unsqueeze(1)
        x_type = target_scores[0].dtype
        masked_tensor = torch.where(expanded_mask, cls,
                                    torch.tensor(float(0), dtype=x_type, device=pred_scores.device))
        result_tensor = torch.amax(masked_tensor, dim=2)
        ...
        if (feat_tensor is not None):
            loss[3] = self.BCEct(feat_tensor, current_targets[:len(feat_tensor)])
